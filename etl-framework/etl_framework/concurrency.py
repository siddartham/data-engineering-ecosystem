...
class Concurrency(Enum):
    ...
    functions called with `etl_framework.broker.Broker.map` and
    `etl_framework.broker.Broker.starmap`:

    -  `etl_framework.concurrency.Concurrency.NONE`:
       Use sequential processing (no concurrency)
    -  `etl_framework.concurrency.Concurrency.SPARK`:
       Use Apache Spark distributed processing
    -  `etl_framework.concurrency.Concurrency.MULTIPROCESSING`:
       Use the python `multiprocessing` module (part of the core library)
       for parallel processing
    -  `etl_framework.concurrency.Concurrency.FUTURES`:
...
def add_parser_concurrency_argument(
    ...
    - parser (argparse.ArgumentParser): A parser to which we are adding the
      parameter.
    - default (etl_framework.concurrency.Concurrency):
      This should be one of the enumerated values defined by
      `etl_framework.concurrency.Concurrency`
    """
    parser.add_argument(
        "-c",
        "--concurrency",
...
def get_concurrency_from_argument_value(
    ...
    `etl_framework.concurrency.Concurrency` based on a string, and
...
def get_argument_value_from_concurrency(concurrency: Concurrency) -> str:
    ...
    `etl_framework.concurrency.Concurrency`, for command-line usage.

    Parameters:

    - concurrency (etl_framework.concurrency.Concurrency)
    """
