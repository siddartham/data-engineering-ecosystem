...
@lru_cache()
def get_file_system() -> local.Local:
    return local.Local(f"{mkdtemp()}/")
...
class TestLocal(unittest.TestCase):
    ...
    def test_get_url(self) -> None:
        ...
        if os.name == "posix":
            assert url == f"file://{self.file_system.root}{TEST1_CSV}"

    def test_date_partition(self) -> None:
        """
        This method tests use of date partitioning functionality for the
        local file system. CSV1 is first written to a date partition directory,
        then CSV2.
        """
