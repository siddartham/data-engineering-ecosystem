...
[![test](https://github.com/siddartham/my-datastore-etl-wrapper/actions/workflows/test.yml/badge.svg)](https://github.com/siddartham/my-datastore-etl-wrapper/actions/workflows/test.yml)
[![distribute](https://github.com/siddartham/my-datastore-etl-wrapper/actions/workflows/distribute.yml/badge.svg)](https://github.com/siddartham/my-datastore-etl-wrapper/actions/workflows/distribute.yml)
...
```shell
pip3 install 'my-datastore-etl-wrapper[all]'
```
...
1. Follow [these instructions](https://github.com/siddarthm/python-package-template/blob/master/README.md)
...
3. Install "my-datastore-etl-wrapper" in your project's virtual environment,
   ...
   **my-datastore-etl-wrapper[spark,snowflake]**, so you'd install as
   follows:

   ```shell script
   pip install 'my-datastore-etl-wrapper[spark,snowflake]'
   ```

4. In ./setup.cfg, add "my-datastore-etl-wrapper[spark,snowflake]~=1.0"
   ...
   packages =
       sample_etl
   install_requires =
       my-datastore-etl-wrapper[snowflake,spark]~=0.0
...
5. Run `make requirements`. This will update your required package versions
   ...
   example, that's just my-datastore-etl-wrapper), and will update
...
```python
import my_datastore_etl_wrapper.broker
from logging import Logger
from my_datastore_etl_wrapper.broker import Concurrency
from my_datastore_etl_wrapper.utilities import get_print_logger
...
class Work(my_datastore_etl_wrapper.broker.Work):
    """
...
# information/objects.
class Broker(my_datastore_etl_wrapper.broker.Broker):
    """
    Your docstring can look much like the parent class' docstring, but
    if you add any additional parameters please include them:
        ...
        - concurrency (my_datastore_etl_wrapper.concurrency.Concurrency)
          = my_datastore_etl_wrapper.concurrency.Concurrency.SPARK
    ...
    def spark_sql_to_s3_example(self) -> None:
        ...
        `my-sample-etl spark-sql-to-s3-example
    ...
    def spark_table_to_s3_example(self) -> None:
        ...
        This method would be executed with a command looking something like
        `my-sample-etl spark-table-to-s3-example
        {dev|qa|prod|local|test|sole-dev|sole-qa|sole-prod}`.
    ...
    def snowflake_sql_to_s3_example(self) -> None:
        ...
        This method would be executed with a command looking something like
        `my-sample-etl snowflake-sql-to-s3-example
        {dev|qa|prod|local|test|sole-dev|sole-qa|sole-prod}`.
        """
        select_statement: str = (
            "SELECT SEASON_YEAR_CD AS SEASON_YEAR_CODE, "
...
# command `python -m my_sample_etl.__main__
...
PACKAGE_NAME: str = "my-sample-etl"
...
def get_spark_submit_operator(
    ...
    would execute the command `my-sample-etl
    ...
    spark_operator: SparkSubmitOperator = SparkSubmitOperator(
        ...
        className="my_sample_etl.__main__",
    ...
    stages {
        ...
        stage('test') {
            when {
                ...
                anyOf {
                    changeset "my_datastore_etl_wrapper/**"
                    changeset "setup.py"
                    changeset "setup.cfg"
                    changeset "tests/**"
                    buildingTag()
...
### [my_datastore_etl_wrapper.broker](./my_datastore_etl_wrapper/broker.py)
...
### [my_datastore_etl_wrapper.concurrency](./my_datastore_etl_wrapper/concurrency.py)
...
- `my_datastore_etl_wrapper.concurrency.Concurrency.NONE`:
  Use sequential processing (no concurrency)
- `my_datastore_etl_wrapper.concurrency.Concurrency.SPARK`:
  Use Apache Spark distributed processing
- `my_datastore_etl_wrapper.concurrency.Concurrency.MULTIPROCESSING`:
  Use the python `multiprocessing` module (part of the core library)
  for parallel processing

### [my_datastore_etl_wrapper.config](./my_datastore_etl_wrapper/config.py)

This module holds public constants for use by this package and dependants.

### [my_datastore_etl_wrapper.transformer](./my_datastore_etl_wrapper/transformer.py)
...
### [my_datastore_etl_wrapper.utilities](./my_datastore_etl_wrapper/utilities.py)
